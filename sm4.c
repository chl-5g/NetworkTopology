 ...
 *
 * 本文件为 GmSSL 的 SM4 小体积实现摘录（ENABLE_SMALL_FOOTPRINT），仅保留
 * ECB 单块与 CBC 分组加解密所需符号。
 */
#define ENABLE_SMALL_FOOTPRINT 1
...
#define S32(A)                                                               \
    ((uint32_t)S[((A) >> 24)] << 24 | (uint32_t)S[((A) >> 16) & 0xff] << 16 | \
     (uint32_t)S[((A) >> 8) & 0xff] << 8 | (uint32_t)S[(A)&0xff])
